...
机器可校验版本：`02-DATA-MAPPING-TABLE.json`
上游依据：`01-DATA-FIELD-CONFIRMATION.md`、`docs/01-core-contracts.md`、`schemas/`
版本：v1.1.0（2026-09-17 修订）

> **2026-09-17 修订摘要**：① 新增 5.1.1 赠品 SKU 别名对照（赛事 `XLG5004` ↔ 演示资产 `GIFT-B5-MASK-2`）；② `order.items[].fulfillment_item_id` 的赠品行改用团队约定 `{订单号}-GIFT-01`；③ OI-02 状态由「待裁决」改为「已收口」。其余映射规则未变。
...
#### 5.1.1 赠品 SKU 别名对照（赛事口径 ↔ 演示口径）

团队实现后，赠品出现了两种写法，指向同一商品。跨线比对时必须按**赠品名称**对齐，不能按 `sku_id` 字面量比对：

| 赠品名称 | 赛事工单口径 | 演示资产口径 | 演示 `fulfillment_item_id` |
|---|---|---|---|
| 测试B5面膜体验装2片 | `XLG5004` | `GIFT-B5-MASK-2` | `6920185815517983396-GIFT-01` |

该对照已在 `02-DATA-MAPPING-TABLE.json` 的 `product_dictionary.gift_sku_alias_competition_vs_demo` 中以机器可读形式给出。
...
| OI-02 | 赠品 SKU 代号不一致 | 赛事字典 `XLG5004` ↔ `fixtures/demo-cases.json` 的 `GIFT-B5-MASK-2` | D 线验收比对歧义 | **已收口（2026-09-17）：保留双写 + 建立别名对照，见 5.1.1** |
...
| OI-05 | 赠品列与客服口述冲突 6 例 | `01-DATA-FIELD-CONFIRMATION.md` 第 6.2 节 | 赠品归属可能被聊天口述带偏 | 产品负责人 |
...
数据来源与标注口径见 `05-EVIDENCE-SOURCE-STATEMENT.md`；英雄案例证据见 `06-HERO-CASE-EVIDENCE-PACKAGE.md`。
