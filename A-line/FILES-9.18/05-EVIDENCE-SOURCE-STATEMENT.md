...
整理人：A 线 数据与调研
日期：2026-09-10
修订日期：2026-09-17（v1.1.0：五张团队测试图片已交付并完成 A 线文件级核验）
...
| `HUMAN_ANNOTATED` | 人工标注 | `03-FIRST-BATCH-ANNOTATIONS-20-CASES.json` 等 | 评测 Ground Truth | 只能用于评测，不得进入运行输入 |
...
|---|---:|---|---|
| `refund_screenshot` | 6 | 退款/打款信息截图 | 否（见 `01-DATA-FIELD-CONFIRMATION.md` 6.3） |
...
**使用限制：** 目录名只能作为粗分类线索，**不得**作为 `ImageObservation` 的证据事实。依据 `docs/02-ai-extraction.md` 第 4.1 节与 `01-DATA-FIELD-CONFIRMATION.md` 第 6.3 节。
...
| 文件名 | 用途 | `source_kind` | 对应赛事路径 | 状态（2026-09-17） |
|---|---|---|---|---|
| `s00001-product-overview.jpg` | 商品整体（SKU 与正装识别） | `TEAM_SYNTHETIC_RECREATION` | `mock_images/broken_pump/S00001_03.jpg` | **已交付 2026-09-12** |
| `s00001-pump-detail.jpg` | 泵头损坏细节 | `TEAM_SYNTHETIC_AUGMENTATION` | — | **已交付 2026-09-12** |
| `s00001-package-context.jpg` | 包装环境 | `TEAM_SYNTHETIC_AUGMENTATION` | — | **已交付 2026-09-12** |
| `s00001-gift-evidence.jpg` | 赠品证据（范围变化用） | `TEAM_SYNTHETIC_AUGMENTATION` | — | **已交付 2026-09-12** |
| `s00001-blurred-pump.jpg` | 模糊图片（人工复核用） | `TEAM_SYNTHETIC_AUGMENTATION` | — | **已交付 2026-09-12**（构图核查结论见 06 第 6 节） |

五张图均由团队用生成式工具自制，均为 1254×1254 RGB JPEG，不含品牌标识、个人信息与消费者材料。A 线已完成文件级核验（存在性、可解码、尺寸、sha256、清晰度），逐图事实与核验结果登记在 `06-HERO-CASE-EVIDENCE-PACKAGE.json`。

由交付素材确定性派生的两张答辩级资产（A 线生成，不替换原图）：

| 文件名 | 用途 | 派生方式 | 位置 |
|---|---|---|---|
| `s00001-pump-detail-blurred.jpg` | 与泵头特写同构图的模糊版本，用于「同角度、只差清晰度」的对照 | 对 `s00001-pump-detail.jpg` 施加高斯模糊，使清晰度指标与已交付模糊图对齐 | `EVIDENCE-ASSETS/` |
| `s00001-pump-detail-assertion.jpg` | 图片内文字断言测试（验收 A26） | 在 `s00001-pump-detail.jpg` 上叠加一行断言文字，其余像素不变 | `EVIDENCE-ASSETS/` |
...
有争议的口径（见 `02-DATA-MAPPING-TABLE.md` 第 9 节）：
...
| 项 | 内容 |
|---|---|
| 文件 | `03-FIRST-BATCH-ANNOTATIONS-20-CASES.json`、`03-FIRST-BATCH-ANNOTATIONS-20-CASES.csv`、`03-CASE-SELECTION-LIST.json` |
| 规模 | 30 例选定（20 开发回归 + 10 留出），首批已完成 20 例 |
| 标注人 | A 线（第一遍，`FIRST_PASS`） |
| 标注规则版本 | `A-LABELING-1.1.0`（2026-09-17 起 H1 优先级按 350 记账） |
| 英雄案例证据 | `06-HERO-CASE-EVIDENCE-PACKAGE.md/.json` + `EVIDENCE-ASSETS/` |
...
| 项 | 状态 | 存放位置 |
|---|---|---|
| 消费者短问卷（目标约 100 份） | 进行中，尚未执行 | `04-RESEARCH-PLAN-AND-RECORDS.md` 第 3、7 节 |
...
| 1.1.0 | 2026-09-17 | ① 五张团队测试图片状态由「待制作」改为「已交付 2026-09-12」，补充文件级核验说明；② 新增两张派生答辩级资产；③ 人工标注数据补充规则版本与英雄案例证据指向；④ 文件名按团队英文命名规范调整 |
