...
上游依据：`docs/06-evaluation-and-research.md` 第 3 节（30 例标注要求）、`docs/03-firewall-rules.md`（规则与优先级，**2026-09-13 起 H1 = 350**）、`schemas/`
版本：v1.1.0（2026-09-17 修订）

> **2026-09-17 修订摘要**：① 判定顺序表与 `rule_priority` 取值同步为 `P0(400) → H1(350) → E1(300) → E2(100) → E0(0)`；② 文件名与 A 线英文命名规范对齐。字段定义本身未变。

标注产物：`03-FIRST-BATCH-ANNOTATIONS-20-CASES.json`、`03-FIRST-BATCH-ANNOTATIONS-20-CASES.csv`、`03-CASE-SELECTION-LIST.json`
...
| `rule_priority` | int | `400` / `350` / `300` / `100` / `0`（2026-09-13 起 H1 = 350） |
...
同一映射表也已写入 `03-FIRST-BATCH-ANNOTATIONS-20-CASES.json` 的 `contract_value_mapping` 节点，供 D 线直接读取。
...
| 3 | `H1` | 350 | `evidence_status = NEED_HUMAN_REVIEW` 或 `issue_type = ADVERSE_REACTION` | `HUMAN_REVIEW` |
...
赛事数据里同一 `scene_minor` 的多个会话共用同一套客服话术模板（见 `01-DATA-FIELD-CONFIRMATION.md` 6.4）。为避免同模板泄漏导致评测虚高：
...
### 5.2 覆盖要求（已由 `03-CASE-SELECTION-LIST.json` 的 `coverage_matrix` 逐项核验）
...
- 留出集案例 ID 已锁定（见 `03-CASE-SELECTION-LIST.json` 的 `held_out`），但不提供答案。
...
字段若需变更，必须同步更新 `03-FIRST-BATCH-ANNOTATIONS-20-CASES.json` 的 `labeling_rules_version`，并在群内同步后再修改，符合 `TEAM_AND_TIMELINE.md` 第 5 节协作约定。
